  ...
  group('Binary BLOB Wire Handling Verification', () {
    ...
    test(
      ...
      () {
        ...
        check(row.typedColByName<Uint8List?>('payload'))
            .isNotNull()
            .deepEquals(rawPayload);
      },
    );

    test('inbound VARCHAR with binary collation (charset 63) is preserved as raw bytes', () {
    ...
    test('inbound prepared statement binary row packet preserves VARCHAR (charset 63) raw bytes', () {
      final varbinColDef = MySQLColumnDefinitionPacket(
        catalog: 'def',
        schema: 'db',
        table: 'tbl',
      ...
      writer.writeUint8(0); // null bitmap (1 byte for 1 col + 2 bits = 3 bits)
      writer.writeVariableEncInt(rawPayload.length);
      writer.write(rawPayload);

      final binRowPkt = MySQLBinaryResultSetRowPacket.decode(writer.toBytes(), [
        varbinColDef,
      ]);
      final row = ResultSetRow.decode(
        colDefs: [varbinColDef],
    ...
    test('typedAssoc preserves binary collation VARCHAR as Uint8List without throwing TypeError', () {
      final varbinColDef = MySQLColumnDefinitionPacket(
        catalog: 'def',
        schema: 'db',
        table: 'tbl',
    ...
    test('accessing negative column indices throws MySQLClientException instead of RangeError', () {
    ...
    test('typedAssoc maps non-binary TEXT columns (blobType w/ charset 33) to String', () {
    ...
    test('typedColAt / typedColByName return Uint8List for dynamic/Object on binary columns', () {
      final blobColDef = MySQLColumnDefinitionPacket(
        catalog: 'def',
        schema: 'db',
        table: 'tbl',
      ...
      check(row.typedColByName<dynamic>('data') as Uint8List)
          .deepEquals(rawPayload);
      check(row.typedColByName<Object>('data') as Uint8List)
          .deepEquals(rawPayload);
      check(row.typedColByName<Object?>('data') as Uint8List)
          .deepEquals(rawPayload);
    });
